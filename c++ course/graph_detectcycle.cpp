    ...
    bool dfsHelper(T source, unordered_set<T> &visited, T parent)
    {
        ...
        for (T neighbour : neighbourList)
        {
            if (visited.find(neighbour) == visited.end())
            {
                ...
                {
                    // you've found a cycle in the sub-components of neighbour hence cycle is present in the given component
                    return true;
                }
            }
            else
            {
                ...
                // check if source to neighbour is a backedge or not ?
                if (neighbour != parent)
                {
                    // source to neighbour is a backedge
                    return true;
                }
            }
        }
...
int main()
{
    ...
    g.isCyclePresent() ? cout << "cycle found!" << endl : cout << "cycle not found!" << endl;
