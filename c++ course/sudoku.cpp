...
bool canPlaceDigit(int grid[][9], int n, int i, int j, int d)
{
    ...
    for (int k = 0; k < n; k++)
    {
        ...
        {
            // you've found digit d in the ith row or jth column
            return false;
        }
    }
    ...
    // check if digit d is present in the subgrid in which the i,jth cell is present

    int rn = sqrt(n); // dimension of the subgrid is rn x rn

    // iterate over the subgrid in which i,jth cell is present to check validity of digit d
    ...
    for (int x = sr; x < sr + rn; x++)
    {
        for (int y = sc; y < sc + rn; y++)
        {
            if (grid[x][y] == d)
            {
                // you've found digit d in the subgrid of the i,jth cell
                return false;
            }
        }
    }
